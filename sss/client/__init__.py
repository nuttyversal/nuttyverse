...
def upload_media(database: psycopg2.extensions.connection, media_id: str, captured_at: datetime.datetime, original_object_id: str, compressed_object_id: str, preview_object_id: str, width: int, height: int, description: str) -> None:
	"""
	...
	with database.cursor() as cursor:
		query = """
			INSERT INTO media (id, captured_at, original_object_id, compressed_object_id, preview_object_id, width, height, description)
...
def update_media(database: psycopg2.extensions.connection, media_id: str, compressed_object_id: str, preview_object_id: str, width: int, height: int) -> None:
	"""
	...
	with database.cursor() as cursor:
		query = """
			UPDATE media
			SET compressed_object_id = %s, preview_object_id = %s, width = %s, height = %s
			WHERE id = %s
		"""

		values = (compressed_object_id, preview_object_id, width, height, media_id)
		cursor.execute(query, values)
		database.commit()
